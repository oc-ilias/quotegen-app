...
jest.mock('@/lib/supabase', () => ({
  createQuote: jest.fn((quote) => Promise.resolve({ ...mockQuote, ...quote })),
  getQuotes: jest.fn((shopId) => Promise.resolve([mockQuote])),
  updateQuoteStatus: jest.fn((quoteId, status, adminNotes, quoteAmount) => 
    Promise.resolve({ ...mockQuote, status, admin_notes: adminNotes, quote_amount: quoteAmount })
  ),
}));
